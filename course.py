...
class CourseClass:
    def __init__(self,root,dash=None):
        ...
        self.course_table.column("description",width=170)
...
#..................................................................
    ...
    def delete(self):
        ...
        try:
            if self.var_course.get()=="":
                ...
                cur.execute("select * from course where name=? and studentemail=?",(self.var_course.get(),self.Email))
                row=cur.fetchone()
                if row==None:
        ...
        try:
            if self.var_course.get()=="":
                ...
                if row!=None:
                    ...
                    cur.execute("insert into course (name,duration,charges,description,studentemail) values(?,?,?,?,?)",(self.var_course.get(),self.var_duration.get(),self.var_charges.get(),self.txt_description.get("1.0",END),self.Email))
                    content.commit()
        ...
        try:
            if self.var_course.get()=="":
                ...
                if row==None:
                    ...
                    cur.execute("update course set name=?,duration=?,charges=?,description=? where name =? and studentemail=?",(self.var_course.get(),self.var_duration.get(),self.var_charges.get(),self.txt_description.get("1.0",END),self.var_course.get(),self.Email))
                    content.commit()
                    content.close()
    ...
    def show(self):
        ...
        cur=content.cursor()
        try:
            cur.execute("select * from course where studentemail=?",(self.Email,))
            rows=cur.fetchall()
    ...
    def search(self):
        ...
        cur=content.cursor()
        try:
            cur.execute(f"select * from course where studentemail='{self.Email}'  and name LIKE '%{self.var_search.get()}%'")
            rows=cur.fetchmany()
            print(rows,"hello" ,self.var_search.get())
            self.course_table.delete(*self.course_table.get_children())
            for row in rows:
                self.course_table.insert('',END,values=row)
